...
use openferric::engines::fft::{CarrMadanParams, HestonCharFn, carr_madan_fft, interpolate_strike_prices};
...
fn main() {
    ...
    let market = Market::builder().spot(spot).rate(rate).dividend_yield(q).flat_vol(0.2).build().unwrap();

    let strikes = [300.0,400.0,500.0,700.0,1000.0,1500.0,2000.0,3000.0,5000.0,8000.0,12000.0];
    let fft = interpolate_strike_prices(&slice, &strikes);
    for (k, fft_px) in fft {
        let gl = engine.price(&VanillaOption::european_call(k, maturity), &market).unwrap().price;
