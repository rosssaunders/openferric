...
mod tests {
    ...
    fn assert_reference(
        ...
        // Haug publishes these cases to four decimals.  `expected` is the
        // full NPV independently generated by QuantLib 1.43's
        // AnalyticBinaryBarrierEngine.  The 1.5e-11 bound covers the measured
        // fourth-order strike-derivative error across Linux and Windows/MSVC
        // (maximum 1.335e-11); it is not an economic price tolerance.
        let error = (value - expected).abs();
        assert!(
            error <= 1.5e-11,
            "binary barrier value {value:.17} differs from QuantLib {expected:.17} by {error:.3e}"
        );
    }

    #[test]
    fn matches_full_haug_quantlib_cash_binary_grid() {
        let barrier = 100.0;
        let cash = 15.0;
        let q = 0.0;
        let r = 0.10;
