...
fn main() {
    ...
    let vg_prices = vg
        .european_calls_fft(spot, &strikes, rate, 0.0, 1.0, params)
        .unwrap();
    println!("Variance-Gamma:");
    for (k, p) in &vg_prices {
        println!("  K={k:6.1}  C={p:.4}");
    }
    ...
    let cgmy_prices = cgmy
        .european_calls_fft(spot, &strikes, rate, 0.0, 1.0, params)
        .unwrap();
    println!("\nCGMY (C=1, G=5, M=10, Y=0.5):");
    for (k, p) in &cgmy_prices {
        println!("  K={k:6.1}  C={p:.4}");
    }
