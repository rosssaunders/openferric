...
use openferric::engines::fft::{BlackScholesCharFn, CarrMadanParams, HestonCharFn, carr_madan_price_at_strikes};
...
fn main() {
    ...
    let fft_prices = carr_madan_price_at_strikes(&cf, rate, expiry, spot, &strikes, CarrMadanParams::default()).unwrap();
    println!("\nCarr-Madan FFT (BS):");
    ...
    let heston_prices = carr_madan_price_at_strikes(&heston_cf, rate, expiry, spot, &strikes, CarrMadanParams::default()).unwrap();
    println!("\nHeston FFT:");
